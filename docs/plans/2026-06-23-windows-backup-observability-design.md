...
```
bash.exe -l -c "bash /c/Users/me/projects/claude-code-backup-guide/scripts/backup.sh /c/Users/me/claude-code-backup"
```
...
2. Reconfigure the existing "Claude Code Backup" task: set its action to
   ...
   Preserve existing triggers (07:00 / 19:00) and principal (`me` / Interactive / Limited).
3. Create or update a "Claude Code Backup Watchdog" task: action
   ...
   triggers: AtLogon **and** a repetition every 4h; principal `me` / Interactive / Limited.
