...
```powershell
. C:\Users\jtkli\projects\claude-code-backup-guide\scripts\windows\toast.ps1
...
```powershell
& C:\Users\jtkli\projects\claude-code-backup-guide\scripts\windows\backup-wrapper.ps1
"exit=$LASTEXITCODE"
...
& C:\Users\jtkli\projects\claude-code-backup-guide\scripts\windows\backup-watchdog.ps1
Get-EventLog -LogName Application -Source ClaudeCodeBackup -Newest 1 | Format-List EventID,EntryType,Message
```
...
& C:\Users\jtkli\projects\claude-code-backup-guide\scripts\windows\backup-wrapper.ps1 | Out-Null
& C:\Users\jtkli\projects\claude-code-backup-guide\scripts\windows\backup-watchdog.ps1
```
...
Run **as Administrator**:
```powershell
& C:\Users\jtkli\projects\claude-code-backup-guide\scripts\windows\install.ps1
...
& C:\Users\jtkli\projects\claude-code-backup-guide\scripts\windows\install.ps1   # run twice — must not error
```
Expected: backup task action is now `powershell.exe ... backup-wrapper.ps1 -BackupDir ...`; its 07:00/19:00 triggers and `jtkli`/Interactive principal are unchanged (`Get-ScheduledTask "Claude Code Backup" | % Triggers` / `.Principal`); watchdog task exists with AtLogon + 4h repetition; second run completes without error.
...
```bash
cd /c/Users/jtkli/projects/claude-code-backup-guide
