...
Expected: `mcp__ssh-mcp-host01__exec` becomes `mcp__<server>__exec`, `mcp__ssh-mcp-host01__sudo-exec` becomes `mcp__<server>__sudo-exec`. Generic permissions like `Bash(git:*)` unchanged. Duplicates removed.
...
- Server names preserved (`secrets`, `ssh-mcp-host01`, etc.)
