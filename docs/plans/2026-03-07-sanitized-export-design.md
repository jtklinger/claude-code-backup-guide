...
- `permissions.allow`: Keep non-MCP entries as-is. For `mcp__*` entries, replace server name with `<server>` (e.g., `mcp__ssh-mcp-host01__exec` → `mcp__<server>__exec`). Deduplicate resulting patterns.
